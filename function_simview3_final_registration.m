...
function function_simview3_final_registration(pathImPattern, TM, baseRegistrationFolder, PSFfilename, transposeOrigImage, ROI)
...
% %     weightsCell{ii} = imwarp(weightsCell{ii}, affine3d(A), 'Outputview', imref3d(imSizeRef), 'interp', 'linear');
    ...
    if( isempty(ROI) == false )
        imCell{ii} = imCell{ii}(ROI(1,1):ROI(2,1), ROI(1,2):ROI(2,2), ROI(1,3):ROI(2,3));
        weightsCell{ii} = weightsCell{ii}(ROI(1,1):ROI(2,1), ROI(1,2):ROI(2,2), ROI(1,3):ROI(2,3));
    end
