...
TMvec = 2700; %:3250; %time points to be registered
...
imFilenameCell = {...
    ['SPM00_TM??????_CM00_CHN01.klb'],...
    ['SPM00_TM??????_CM02_CHN00.klb']};
 %   ['SPM00_TM??????_CM01_CHN01.klb'],...
 %   ['SPM00_TM??????_CM03_CHN00.klb']};%filenames of each view. CRITICAL: the same order should be preserved when giving the transformations to match images between cameras
...
cameraTransformCell = [10, 20];

TrCellPre = []; %{[0 0 0], 4*[-8 -3 -138], 4*[-34 2 0], 4*[-9 -1 -5]};  %Translation for each camera after the camera transform cell in order to perform coarse alignmnet. Leave empty if you want to automatically calculate it using normalized cross correlation

samplingXYZ = [0.40625, 0.40625, 6.780];%sampling in um

FWHMpsf = [0.8, 0.8, 4.0]; %theoretical full-width to half-max of the PSF in um.

outputFolderPattern = ['D:\temp\registration\dr_15_05_05-ngc\TM??????_localMaxImgBin_view_0_1\'];%outputfolder for debugging purposes. Leave empty for no debugging. This folder should be visible to the cluster if you want to run it on it
...
RANSACparameterSet.minIntensityValue = 110; %global threshold. Any pixel below that intesnity will not be considered a point of interest for matching
...
RANSACparameterSet.searchRadius = 96;      %maximum distance (in pixels) between two views to match corresponding points after coarse alignment. If coarse alignment works well, this can be small. The smaller the value, the less memory is required.
...
RANSACparameterSet.thrNCC = 0.7;            %threshold of NCC to accept a match
...
RANSACparameterSet.thrMask = RANSACparameterSet.minIntensityValue;           %global threshold to find "embryo mask" (so we can exclude beads from registration). Only needed for DoG detector
