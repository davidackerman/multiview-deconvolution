...
outputFolderPattern = ['T:\temp\registration\dros_GCaMP6s_15-08-22_diSPIM\TM??????_localMaxImgBin\'];%outputfolder for debugging purposes. Leave empty for no debugging. This folder should be visible to the cluster if you want to run it on it
...
RANSACparameterSet.minIntensityValue = 170; %global threshold. Any pixel below that intesnity will not be considered a point of interest for matching
...
RANSACparameterSet.thrMask = 170;           %global threshold to find "embryo mask" (so we can exclude beads from registration). Only needed for DoG detector
