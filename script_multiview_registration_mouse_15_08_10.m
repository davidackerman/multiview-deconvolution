...
TrCellPre = {[0 0 0], 4 * [28 -3 -66], 4*[-34 2 0], 4*[27 -1 -58]};  %Translation for each camera after the camera transform cell in order to perform coarse alignmnet. Leave empty if you want to automatically calculate it using normalized cross correlation

samplingXYZ = [0.40625, 0.40625, 2.031];%sampling in um

FWHMpsf = [0.8, 0.8, 3.0]; %theoretical full-width to half-max of the PSF in um.

outputFolderPattern = ['T:\temp\registration\mouse_15_08_10\TM??????_localMaxImgBin\'];%outputfolder for debugging purposes. Leave empty for no debugging. This folder should be visible to the cluster if you want to run it on it
...
RANSACparameterSet.thrMask = 180;           %global threshold to find "embryo mask" (so we can exclude beads from registration). Only needed for DoG detector
