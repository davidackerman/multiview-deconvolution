...
for i_trial = 1:n_trials ,
    ...
    % Compute the transformed PSF in C++
    transformed_psf_cpp = ...
        run_cpp_imwarp_flexible(input_stack_single, input_origin_single, input_spacing_single, ...
                                   T_in_row_form_single, ...
                                   output_origin_single, output_spacing_single, output_dims_uint64) ;                               
        
    assert(isa(transformed_psf_matlab, 'single')) ;   
    assert(isa(transformed_psf_cpp, 'single')) ;   

    % Trim the psf in matlab
    trimmed_psf_matlab = run_matlab_trim_psf(transformed_psf_matlab) ;
    
    % Trim the psf in C++
    trimmed_psf_cpp = run_cpp_trim_psf(transformed_psf_cpp) ;

    assert(isa(trimmed_psf_matlab, 'single')) ;   
    assert(isa(trimmed_psf_cpp, 'single')) ;   
